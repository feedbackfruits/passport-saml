...
SAML.prototype.getCallbackUrl = function (req, authorizeOptions = {}) {
    // Post-auth destination
  if (authorizeOptions.callbackUrl) {
    console.log('SAML -- ','Getting callbackUrl from authorizeOptions:', authorizeOptions.callbackUrl);
    return authorizeOptions.callbackUrl;
  } else {
...
SAML.prototype.generateAuthorizeRequest = function (req, isPassive, authorizeOptions, callback) {
  var self = this;
  ...
  .then(function(){
    var request = {
      'samlp:AuthnRequest': {
        ...
        '@AssertionConsumerServiceURL': self.getCallbackUrl(req, authorizeOptions),
...
SAML.prototype.getAuthorizeUrl = function (req, callback, authorizeOptions) {
  var self = this;
  self.generateAuthorizeRequest(req, self.options.passive, authorizeOptions, function(err, request){
    if (err)
      return callback(err);
